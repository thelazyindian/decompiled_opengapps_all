...
var InlineImageDeletionObserver;
...
/**
 * Registers a mutation observer to observe any inline image deletions
 */
registerInlineImageDeletionObserver = function() {
  var target = document.querySelector(DIV_BODY);
  if (!InlineImageDeletionObserver) {
    InlineImageDeletionObserver = new MutationObserver(function(mutations) {
      if (!InlineImageDeletionObserver) {
        return;
      }

      // Check for deleted inline images
      var urls = [];
      mutations.forEach(function(mutation) {
        var node;
        for (var i = 0; node = mutation.removedNodes[i]; ++i) {
          ...
          if (node.tagName.toLowerCase() == 'img') {
            urls.push(getImageSrc(node));
          } else {
            var imageElements = node.getElementsByTagName('img');
            for (var i = 0; i < imageElements.length; i++) {
              urls.push(getImageSrc(imageElements[i]));
            }
          }
        }
      });
      if (urls.length > 0) {
        BodyChangeListener.deleteInlineAttachments(urls);
      }
    });
  }
  InlineImageDeletionObserver.observe(target, {childList: true, subtree: true});
};
...
registerMutationObservers = function() {
    ...
    registerInlineImageDeletionObserver();
...
/**
 * Unregisters inline image deletion mutation observer.
 */
unregisterInlineImageDeletionObserver = function() {
  if (InlineImageDeletionObserver) {
    InlineImageDeletionObserver.disconnect();
  }
};
...
unregisterMutationObservers = function() {
    ...
    unregisterInlineImageDeletionObserver();
...
/**
 * Get the src of the inline image
 * @param {Element} node Image node
 * @return {string} the src of the image node
 * @private
 */
function getImageSrc(node) {
  var blockedSrc = node.getAttribute(BLOCKED_SRC_ATTR);
  return blockedSrc ? blockedSrc : node.src;
}
